...
string codeGenerator::generateVerilog(
    std::unique_ptr<simpleParser::netlistDefiniton> &netlist_ptr) {
  std::string verilogText;
  std::set<std::string> netNames; // wires is set to avoid repetitions
  std::vector<std::string>
      inputNames; // inputs buffer to filter them out from wires
  std::vector<std::string>
      outputNames; // outputs buffer to filter them out from wires
  ...
  for (const auto &gate : netlist_ptr->gates) {
    for (const auto &net : gate.mPins) {
      if (find(inputsPtr->begin(), inputsPtr->end(), net.mPin) ==
              inputsPtr->end() &&
          find(outputsPtr->begin(), outputsPtr->end(), net.mPin) ==
              outputsPtr->end()) {
  ...
  for (int input = 0; input < netlist_ptr->inputs.size(); input++) {
    ...
    if (input % 8 == 7 || input == netlist_ptr->inputs.size() - 1) {
      ...
      verilogText +=
          netlist_ptr->inputs[input].mName + ";\n"; // Add semicolon and newline
    } else {
  ...
  for (auto &gate : netlist_ptr->gates) {
    ...
    for (int i = 0; i < gate.mPins.size(); i++) {
      if (i == gate.mPins.size() - 1) {
        ...
        // ")";
        verilogText +=
            "." + gate.mPins[i].mName + "(" + gate.mPins[i].mPin + ")";
